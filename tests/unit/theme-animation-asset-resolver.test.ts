...
import {
  ThemeAnimationAssetResolver,
} from "../../src/theme/ThemeAnimationAssetResolver";
...
describe(
  "ThemeAnimationAssetResolver",
  () => {
    it("resolves animation assets from the theme", () => {
      const theme = createTheme();

      const resolver =
        new ThemeAnimationAssetResolver(theme);

      expect(
        resolver.resolveAnimationAssets()
      ).toEqual({
        frame01HighRes:
          "/themes/sample-theme/animation/frame-01.webp",
